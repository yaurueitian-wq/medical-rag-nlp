...
### 11.5 實驗追蹤架構（Kaggle 風格）

參考 Kaggle competition 的做法，建立了結構化的實驗追蹤系統：

```
experiments/
├── optimization_log.md         # Leaderboard 總覽表
├── v0_baseline/                # 每個版本獨立資料夾
│   ├── config.json             # 參數快照
│   ├── results.json            # 完整評估結果
│   └── notes.md                # 變更說明與觀察
├── v1_xxx/
...
| 元素 | 對應 Kaggle | 說明 |
|------|------------|------|
| `test_questions.json` | 測試資料集 | 10 題固定問題 + ground truth |
| `experiments/vN_xxx/` | 每次 submission | 獨立保存參數 + 結果 |
| `optimization_log.md` | Leaderboard | 所有版本分數一覽 |
| `eval_rag.py` | 評分腳本 | 自動執行評估、存結果 |

### 11.6 Level 0 實作：評估框架（2026-02-05）

#### 新增檔案

| 檔案 | 功能 |
|------|------|
| `medical_data.py` | 共用醫療資料模組（避免 rag_app.py 和 eval_rag.py 重複定義） |
| `test_questions.json` | 10 題標準測試集（涵蓋 6 份文件 + 跨科別綜合題） |
| `eval_rag.py` | 評估腳本，自動跑測試、算分數、存結果 |
| `experiments/` | 實驗追蹤目錄 |
...
| 指標 | 說明 | 評估方式 |
|------|------|---------|
| **Retrieval Hit Rate** | 檢索結果包含預期來源的比例 | 自動比對 metadata |
| **Retrieval MRR** | 預期來源排名的倒數平均 | 自動計算 |
| **平均相似度** | 檢索片段的 cosine similarity 平均 | ChromaDB 提供 |
| **關鍵字覆蓋率** | 回答包含預期關鍵字的比例 | 自動字串比對 |
| **忠實度** | 回答是否忠實於參考資料 (1-5) | LLM-as-Judge |

#### v0 Baseline 結果

| 指標 | 數值 | 分析 |
|------|------|------|
| Retrieval Hit Rate | **80.0%** | 8/10 題找到正確來源 |
| Retrieval MRR | **0.533** | 正確來源不一定排第一 |
| 平均相似度 | **52.8%** | 整體偏低，有改善空間 |
| 關鍵字覆蓋率 | **50.0%** | 一半的預期關鍵字未出現在回答中 |
| 忠實度 | **3.5/5** | 中等，回答大致基於參考資料 |

#### Baseline 弱點分析

- **Q1 糖尿病診斷標準**：檢索未命中「內科學教科書」，Top-1 相似度僅 51.3%
- **Q6 MONA 治療**：檢索命中但關鍵字覆蓋 0%，LLM 未展開 MONA 的完整縮寫
- **Q8 CKD 血壓目標**：檢索未命中「腎臟內科學」
- **Q10 跨科別題**：關鍵字覆蓋 0%，跨文件的資訊整合能力弱

#### 討論決策

- **為什麼用 LLM-as-Judge？** 用同一個本地 3b 模型來評分，雖然準確度不如大模型，但完全離線、不需 API，適合學習環境
- **為什麼 10 題？** 涵蓋全部 6 份文件 + 1 題跨科別，數量適中但有代表性
- **下一步方向：** 根據 baseline 弱點，Level 1 資料層優化（改善分割策略）應該能提升相似度和命中率
