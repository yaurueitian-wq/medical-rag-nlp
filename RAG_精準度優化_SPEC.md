...
### 4.6 輔助訊號：Top-K 分數分佈與回饋機制的協同（已實作）

**緣由：** Top-K 檢索回傳的相似度分數，其排序與分佈本身就帶有「檢索品質」的訊號——例如 Top1 與 Top2 分數差距大（陡降）通常代表 embedding 模型能清楚分辨真正相關的內容；分數普遍偏低或分佈平坦，則可能是 retrieval 品質不佳的前兆。但需明確定位：**這是「輔助、低成本的初步健檢指標」，不能取代使用者回饋或人工標註**，原因：
- 分數分佈只反映候選之間的「相對」關係，無法判斷 Top1 是否「絕對」正確
- 分數異常可能來自多種原因（embedding 模型不適合／chunk 切得不好／知識庫缺漏），無法單從分數歸因到具體類別，仍需 `failure_type` 的人工標註才能確定
- 分數高不保證生成的答案符合使用者體感，這是檢索面與生成面的落差

**實作方式（已完成，[rag_app.py:64-87](rag_app.py#L64-L87)）：** 新增 `compute_score_distribution()`，在 `save_feedback()` 寫入時，從該次查詢的 `retrieved_sources` 計算並附加：
- `top1_score`：最高分
- `score_gap`：Top1 與 Top2 的分數差距（陡降 vs. 平坦的量化指標）
- `avg_score`：平均分數
- `score_std`：分數標準差（離散程度）

**範圍界定：** 此統計**僅附加在使用者已主動開啟回饋（toggle）並送出評分的紀錄上**，不擴及未開啟回饋的查詢。原因：若擴大到「不論是否開啟回饋，系統都默默記錄每次查詢的分數分佈」，即使不含問題文字內容，仍是另一種形式的被動資料蒐集，與第 1.1 節「回饋機制不能犧牲使用者體驗／不應強加在一般使用流程中」的設計原則相衝突。是否要做這種被動監測（樣本量更大，但需要重新評估隱私權衡），留待第 5 節待決策事項，不在本次範圍內自動實作。

**後續應用：** 待 `feedback_log.jsonl` 累積足夠資料後，分析 `score_distribution` 與 `failure_type` 的相關性（例如：標記為 A 類的案例，`score_gap` 是否確實普遍偏小）。若驗證成立，可進一步用分數分佈**預先建議**最可能的失敗類型（例如使用者按下 👎 時自動帶出建議選項），降低填寫負擔；若不成立，則停止依賴此訊號做自動化判斷。
...
- [ ] 是否要將 Top-K 分數分佈統計（4.6）擴大到「被動監測所有查詢」，不限於已開啟回饋的使用者？需權衡樣本量增加 vs. 與「自願參與」設計原則的衝突
